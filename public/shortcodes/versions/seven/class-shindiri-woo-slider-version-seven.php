    ...
    */
	public function slider_shortcode( $post_id, $meta_values ) {
        ...
        if ( isset( $meta_values[7] ) ) {
            ...
            return esc_html__( 'Something went wrong. Please check slider version metadata settings.', 'woo-shop-slider-lite' );
        }
        ...
        if ( $show_what === 'products' ) {
            ...
            if ( is_wp_error( $the_query ) || empty( $the_query->posts ) ) {
                return Shindiri_Woo_Slider_Helper::slider_public_error( esc_html__( 'No results for current setting found. Please change slider settings', 'woo-shop-slider-lite' ) );
            }
            ...
            if ( !is_array( $cats_slugs ) && empty( $cats_slugs ) ) {
                ...
                if ( !empty( $wc_all_cats ) && is_array( $wc_all_cats ) ) {
                    ...
                    return Shindiri_Woo_Slider_Helper::slider_public_error( esc_html__( 'No product categories found. Please change slider settings', 'woo-shop-slider-lite' ) );
                }
            }
